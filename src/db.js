...
export const fetchReferenceData = async () => {
  ...
  const [joints, sellers, suppliers] = await Promise.all([
    ...
    supabase.from("sellers").select("id,name,joint_id,is_active").order("name"),
...
export const fetchDashboardMetrics = async () => {
  ...
  const totalDeliveries = (deliveries.data ?? []).reduce((sum, row) => sum + row.qty, 0);
  const totalAllocations = (allocations.data ?? []).reduce((sum, row) => sum + row.qty_basis, 0);
  const totalPayments = (payments.data ?? []).reduce(
    (sum, row) => sum + row.amount_ghs / CONFIG.BASIS_UNIT_PRICE,
    0
  );
