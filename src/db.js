...
export const fetchReferenceData = async () => {
  ...
  if (!supabase) return { joints: [], sellers: [], suppliers: [] };
...
export const fetchDashboardMetrics = async () => {
  const supabase = supabaseClient();
  if (!supabase) return { expectedStock: 0, expectedSeller: 0, activity: [] };
  ...
  const totalPayments = (payments.data ?? []).reduce(
    (sum, row) => sum + (row.amount_ghs ?? 0) / (CONFIG.BASIS_UNIT_PRICE || 1),
    0
  );
...
export const listTable = async (table, query = {}) => {
  ...
  if (query.eq) {
    Object.entries(query.eq).forEach(([key, value]) => {
      // bewusst: value kann "" sein, dann nicht filtern
